...
function buildPrompt(mode, intake) {
 ...
 if (mode === "free_report") {
  return [
    ...
    "【文字数】日本語で1200〜1400字（目安：冒頭300字、相性500字、7日以内500字、CTAは短め）。",
    "",
    "【出力形式】HTMLのみ。使用できるタグは <div><p><strong><ul><li> のみ。",
    "・各セクションは <div class='sec'> で区切る。",
    "・見出しは <p><strong>見出し名</strong></p>。",
    "・1段落が長くなりすぎないよう、必ず段落を分ける。",
     "・【改行ルール（重要）】
 "・- 空行は入れない。段落は各セクション最大2段落まで。
 ...
 "・- <p>は最大2個まで。空の<p>は禁止。",
    ...
    "・五行は“味付け”程度。各セクションで0〜1回が目安。『気・運・縁・流れ・間・余白』は自然に入れる。",
    "",
    "【バーナム効果（必須）】",
    "・ユーザー側：強がる→スマホを見る→自分だけ前のめりが怖い→期待しすぎた？と反省、を自然に当てる。",
    "・相手側：返したい気はあるが、返すほど重く感じて止まる／余白がないと返せない／気分と疲れで速度が変わる、を自然に当てる。",
    "",
"【事実の反映（必須）】",
"・入力から“固有の事実”を最低3点抽出して本文に入れる（関係性/回数や期間/連絡状況/直近出来事/感情の揺れ）。",
"・数値があれば最低1つ使う。なければ曖昧表現で補う。",
"・断言は避けつつ、現実的な見立ては必ず入れる。",
"",
    "【各セクションの目安文字数】",
    "・0) 300字前後：寄り添い＋今が揺れる理由＋ここで見たいポイントを宣言。",
    "・1) 500字前後：相性の核を“翻訳”して、2回会えた意味、未読が起きる典型パターン、相手の心のブレーキを具体的に描写。",
    "・2) 500字前後：3日〜7日の間に起きやすい“心の変化”を2〜3パターンで描写（返信の有無は断言しない）。ユーザーの心の揺れも同時に当てる。",
    "",
    "【CTA（短め・でも刺す）】",
    "・無料版では『結論（どう動くか）』はあえて出さない。",
    "・有料版で初めて『一手（送る文面まで）』『吉凶』『次の1ヶ月の流れ』『復縁の確度を上げる間の取り方』を出す。",
    "・300円は“今週の扱い方（1手）と心の癖の修正”。980円は“1ヶ月の流れ＋相手の本音の出やすい日＋復縁の分岐点”。",
    "",
    "【入力データ】",
    `本人の気：${userPair}`,
    `相手の気：${partnerPair}`,
